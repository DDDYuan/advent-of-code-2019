...
while(codes[pointer] != 99):
    currentCode = codes[pointer]
    print('code is :' + str(currentCode))
    if currentCode == 3:
        read = input('Input your value:')
        codes[codes[pointer+1]] = int(read)
        pointer += 2
    elif currentCode == 4:
        print('Output is:' + str(codes[codes[pointer+1]]))
        pointer += 2
        continue
    else:
        (opCode,firstType,secondType) = parseCommand(codes[pointer])
        if opCode == 1:
            firstNumber = codes[pointer+1] if firstType == 1 else codes[codes[pointer+1]]
            secondNumber = codes[pointer+2] if secondType == 1 else codes[codes[pointer+2]]
            result = firstNumber + secondNumber
            codes[codes[pointer+3]] = result
        elif opCode == 2:
            firstNumber = codes[pointer+1] if firstType == 1 else codes[codes[pointer+1]]
            secondNumber = codes[pointer+2] if secondType == 1 else codes[codes[pointer+2]]
            result = firstNumber * secondNumber
            ...
            condition = codes[pointer+1] if firstType == 1 else codes[codes[pointer+1]]
            if condition != 0:
                pointer = codes[pointer+2] if secondType == 1 else codes[codes[pointer+2]]
            else:
                pointer += 3
            continue
        elif opCode == 6:
            ...
            firstNumber = codes[pointer+1] if firstType == 1 else codes[codes[pointer+1]]
            secondNumber = codes[pointer+2] if secondType == 1 else codes[codes[pointer+2]]
            codes[codes[pointer+3]] = 1 if firstNumber < secondNumber else 0
        elif opCode == 8:
            firstNumber = codes[pointer+1] if firstType == 1 else codes[codes[pointer+1]]
            secondNumber = codes[pointer+2] if secondType == 1 else codes[codes[pointer+2]]
            codes[codes[pointer+3]] = 1 if firstNumber == secondNumber else 0
